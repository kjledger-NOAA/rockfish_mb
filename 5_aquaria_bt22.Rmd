...
using 'decontaminated'/filtered samples/asv and tax assignment (v.534) 
...
try other ways of plotting 
```{r}
# tank_fig2 <- taxon_table %>%
#   filter(Sample_ID %in% c("e01737", "e01738", "e01741")) %>%
#   group_by(Sample_ID) %>%
#   mutate(reads_per_sample = sum(tot_reads)) %>%
#   mutate(prop_reads = tot_reads/reads_per_sample) %>%
#   ggplot(aes(x=taxon, y=prop_reads, fill=taxon)) +
#   geom_bar(stat = "identity") + 
#   theme_bw() +
...
```{r}
#ggsave("/home/kimberly.ledger/rockfish_mb/figures/tank2_new_cpv_assignments.png", plot = tank_fig2, dpi = 300, width = 7, height = 4)
```

```{r}
# tank_fig3 <- taxon_table %>%
#   filter(Sample_ID %in% c("e01737", "e01738", "e01741")) %>%
#   pivot_wider(names_from = "taxon", values_from = "tot_reads", values_fill = 0) %>%
#   pivot_longer(cols = c(3:13), names_to = "taxon", values_to = "tot_reads") %>%
#   group_by(Sample_ID) %>%
#   mutate(reads_per_sample = sum(tot_reads)) %>%
#   mutate(prop_reads = tot_reads/reads_per_sample) %>%
#   ungroup() %>%
...
#             q.25 = quantile(prop_reads, 0.25),
#             q.75 = quantile(prop_reads, 0.75)) %>%
#   ggplot(aes(x=taxon, y=mean_prop, color=taxon)) +
#   geom_point() + 
#   geom_errorbar(aes(ymin = q.25, ymax = q.75), width = 0.2) + 
#   theme_bw() +
#   labs(
#     y = "relative read abundance (%)",
#     x = "taxon") + 
...
```{r}
#ggsave("/home/kimberly.ledger/rockfish_mb/figures/tank3_new_cpv_assignments.png", plot = tank_fig3, dpi = 300, width = 6, height = 4)
```
...
eDNA pick up a lot of melanosticutus (blackspotted)and some proringer (redstripe). other people at the lab believe these species are also in the tank. 
...
my_colors <- c("#f7754f",  "#6e9bf4", "#cc7af4", "#f565cc", "#77ab31")
...
```{r}
ggsave("/home/kimberly.ledger/rockfish_mb/figures/bt2_w_zeros.png", plot = bt_plot2, dpi = 300, width = 8, height =3)
```
