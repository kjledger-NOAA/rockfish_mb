...
```{r}
tax <- read.csv("~/rockfish_mb/data/taxonomy_custom519_20241106.csv") %>%
  select(!X) %>%
  rename(ASV = qseqid)
```
...
load asv table
```{r}
asv_table <- read.csv("/genetics/edna/workdir/sebastes/tissues/trimmed/filtered/outputs/ASVtable.csv")
```
...
notes: 
* S. diaconus, S. altivelis, and S. microchar do not have reference sequences in the current version of the custom rockfish reference db so that helps explain this mis-ids for those species. 
