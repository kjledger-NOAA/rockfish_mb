...
temp1 <- asv_table %>%
  ...
  summarise(across(ASV9:ASV262, sum, na.rm = TRUE)) %>%
  t() %>%
  as.data.frame()
...
fasta_file <- "/genetics/edna/workdir/sebastes/tissues/trimmed_20241204/filtered/outputs/myasvs.fasta"
output_file <- "/home/kimberly.ledger/rockfish_mb//outputs/deacon.fasta"

# Read the FASTA file
fasta_seqs <- readDNAStringSet(fasta_file)

# Filter the FASTA sequences based on the ASV headers
deacon_seqs <- fasta_seqs[names(fasta_seqs) %in% deacon_filtered]
...
temp1 <- asv_table %>%
  ...
  group_by(common_name) %>%
  summarise(across(ASV75:ASV236, sum, na.rm = TRUE)) %>%
  t() %>%
  as.data.frame()
...
# Define the input and output file paths
fasta_file <- "/genetics/edna/workdir/sebastes/tissues/trimmed_20241204/filtered/outputs/myasvs.fasta"
output_file <- "/home/kimberly.ledger/rockfish_mb/outputs/sunset.fasta"
...
temp1 <- asv_table %>%
  ...
  group_by(common_name) %>%
  summarise(across(ASV77:ASV212, sum, na.rm = TRUE)) %>%
  t() %>%
  as.data.frame()
...
# Define the input and output file paths
fasta_file <- "/genetics/edna/workdir/sebastes/tissues/trimmed_20241204/filtered/outputs/myasvs.fasta"
output_file <- "/home/kimberly.ledger/rockfish_mb/outputs/olive.fasta"

# Read the FASTA file
fasta_seqs <- readDNAStringSet(fasta_file)

# Filter the FASTA sequences based on the ASV headers
olive_seqs <- fasta_seqs[names(fasta_seqs) %in% olive_filtered]
...
temp1 <- asv_table %>%
  ...
  group_by(common_name) %>%
  summarise(across(ASV81:ASV237, sum, na.rm = TRUE)) %>%
  t() %>%
  as.data.frame()
...
# Define the input and output file paths
fasta_file <- "/genetics/edna/workdir/sebastes/tissues/trimmed_20241204/filtered/outputs/myasvs.fasta"
output_file <- "/home/kimberly.ledger/rockfish_mb/outputs/gopher.fasta"

# Read the FASTA file
fasta_seqs <- readDNAStringSet(fasta_file)

# Filter the FASTA sequences based on the ASV headers
gopher_seqs <- fasta_seqs[names(fasta_seqs) %in% gopher_filtered]
